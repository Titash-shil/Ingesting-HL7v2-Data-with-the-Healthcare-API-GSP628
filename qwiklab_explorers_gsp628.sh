...
echo -n "${GREEN_TEXT}${BOLD_TEXT}Please enter the GCP region: ${RESET_FORMAT}"
read REGION
export REGION
...
echo "${YELLOW_TEXT}${BOLD_TEXT}Step 1: Enabling required GCP services...${RESET_FORMAT}"

gcloud healthcare datasets create dataset1 --location=${REGION}

echo "${WHITE_TEXT}${BOLD_TEXT}Step 2: Creating Healthcare dataset...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}Step 4: Creating Pub/Sub topic and subscription...${RESET_FORMAT}"
